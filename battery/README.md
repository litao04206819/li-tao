...
本模块使用 [PyBaMM](https://pybamm.org)（Python Battery Mathematical Modelling）构建锂离子电池模型，
以 Jupyter Notebook 形式由浅入深演示从基础放电仿真到参数辨识的完整链路。

> 说明：PyBaMM 为纯 Python 物理建模库，无 MATLAB 等价实现，因此本模块仅提供 Python / Notebook 版本，
> 不同于仓库其余部分（`p0x`/`m0x`）的 Python + MATLAB 双语言约定。

## 安装

```bash
pip install -r battery/requirements.txt
# 验证
...
```bash
jupyter lab battery/          # 或 jupyter notebook
```

命令行端到端执行（验证可无错跑通）：

```bash
jupyter nbconvert --to notebook --execute battery/nb01_basics.ipynb
```
